...
class StyTR3_L1Losses(pl.LightningModule):
    """
    StyTR-2 model with L1 perceptual losses.
    ...
    def loss_fn(self, style, content, stylized, identity_style, identity_content, reverse_stylized, idx):
        """
        ...
        for l in self.vgg_layers:
            ...
            s_loss += F.l1_loss(mean_t, mean_s) + F.l1_loss(std_t, std_s)
    
        # 4) Identity loss 1 (pixel‐level)
        i_loss1 = F.l1_loss(identity_style, style) + F.l1_loss(identity_content, content)
        ...
        i_loss2 = 0.0
        for l in self.vgg_layers:
            i_loss2 += F.l1_loss(i_s[l], f_s[l]) + F.l1_loss(i_c[l], f_c[l])
        ...
        if idx % 100 == 0:
            print(f"[{idx}] c:{c_loss.item():.4f} "
                  f"s:{s_loss.item():.4f} "
                  f"i1:{i_loss1.item():.4f} "
                  f"i2:{i_loss2.item():.4f} "
    ...
    def training_step(self, batch, idx):
        ...
        reverse_stylized = self(content, style)     # new term
        
        loss = self.loss_fn(style, content, stylized, identity_style, identity_content, reverse_stylized, idx)
